        ...
        for s in session:
            for v in variant:
                for q in qp_or_ms:
                    ...
                    time.sleep(1)
                    url = f"https://pastpapers.co/cie/IGCSE/{subj}/{y}-{s}/0478_{winter_or_summer}_{q}_{v}.pdf"
                    response = requests.get(url)
                    if response.status_code == 200:
                        # Open PDF file in wb mode
                        subfolder = f"{y}-{s}"
                        second_subfolder = "Question_Papers" if q == "qp" else "Mark_Schemes"
                        os.makedirs(os.path.join("0478_Computer_Science", subfolder, second_subfolder), exist_ok=True)
                        with open(os.path.join("0478_Computer_Science",subfolder, second_subfolder, f"{subject_code}_{y}_{s}_{q}_{v}.pdf"), 'wb') as f:
